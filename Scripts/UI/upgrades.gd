...
func _on_more_lasers_pressed() -> void:
	if get_parent().more_lasers_index < 2:
		if get_parent().score > get_parent().more_lasers_cost[get_parent().more_lasers_index]:
			...
			if get_parent().more_lasers_index < 2:
				...
				$vertical_buttons/more_lasers.text = str("MAX")
	else:
		$vertical_buttons/more_lasers.text = str("MAX")

func _on_more_damage_pressed() -> void:
	if get_parent().more_damage_index < 6:
		...
		$vertical_buttons/more_damage.text = str("MAX")


func _on_more_hp_pressed() -> void:
	if get_parent().more_hp_index < 6:
		...
		$vertical_buttons/more_hp.text = str("MAX")
